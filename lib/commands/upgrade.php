...
/**
 * @command		upgrade
 * @desc		Fetch the newest version of {app}
 * @usage		upgrade
 */
$command = new Command\Command( 'upgrade', 'Fetch the newest version of {app}', function()
{
	/* Build the URL slug — ex., https://github.com/example/repo > 'example/repo' */
	$urlPieces = explode( '/', $this->repoURL );
	$urlSlugPieces = array_slice( $urlPieces, -2, 2 );

	if( count( $urlSlugPieces ) != 2 )
	{
		throw new Command\CommandInvokedException( "Cannot perform upgrade: Missing project URL. Please contact the developer to resolve this misconfiguration.", 1 );
	}

	$urlSlug = implode( '/', $urlSlugPieces );
	...
	{
		throw new Command\CommandInvokedException( "The server said: '{$responseStatus['message']} ({$responseStatus['code']})'. Please submit a bug report regarding this failed upgrade, https://github.com/{$urlSlug}/issues", 1 );
	}
